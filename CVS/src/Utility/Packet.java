...
//	};
	...
	//return cmd type
	public static int checkRecInfo(byte [] RecBuf,int recLength,byte [] InfoBuf)
	{
		int headLen = 22;
		int tailLen = 6;
		int InfoLength=0;
		...
		Log.e("check", "rec len"+InfoLength);
	...
	private static char[] getChars(byte[] bytes,int start,int len) {
		Charset cs = Charset.forName("UTF-8");
		ByteBuffer bb = ByteBuffer.allocate(len);
		bb.put(bytes,start,len);
		bb.flip();
		CharBuffer cb = cs.decode(bb);
		return cb.array();
	}
	
	public static List<String> phaseDbInfo(byte [] infoBuf)
	{
		List<String> ls = new ArrayList<String>();
		char[] s1 = getChars(infoBuf,0,20);
		ls.add(String.valueOf(s1));
		//Log.e("check", ls.get(0));
		
		s1 = getChars(infoBuf,20,20);
		ls.add(String.valueOf(s1));
		//Log.e("check", ls.get(1));
		
		s1 = getChars(infoBuf,40,20);
		ls.add(String.valueOf(s1));
		//Log.e("check", ls.get(2));
		
		s1 = getChars(infoBuf,60,20);
		ls.add(String.valueOf(s1));
		//Log.e("check", ls.get(3));
		
		s1 = getChars(infoBuf,80,20);
		ls.add(String.valueOf(s1));
		//Log.e("check", ls.get(4));
